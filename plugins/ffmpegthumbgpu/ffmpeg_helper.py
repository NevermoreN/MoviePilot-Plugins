...
class FfmpegHelper:
    ...
    def extract_wav(video_path: str, audio_path: str, audio_index: str = None):
        """
        使用ffmpeg从视频文件中提取16000hz, 16-bit的wav格式音频，利用GPU加速
        """
        ...
        # 提取指定音频流
        if audio_index:
            command = ['ffmpeg', "-hide_banner", "-loglevel", "warning", '-hwaccel', 'cuda', '-y', '-i', video_path,
    ...
    def extract_subtitle(video_path: str, subtitle_path: str, subtitle_index: str = None):
        ...
        if subtitle_index:
            command = ['ffmpeg', "-hide_banner", "-loglevel", "warning", '-hwaccel', 'cuda', '-y', '-i', video_path,
                       '-map', f'0:s:{subtitle_index}', subtitle_path]
        else:
            command = ['ffmpeg', "-hide_banner", "-loglevel", "warning", '-hwaccel', 'cuda', '-y', '-i', video_path, subtitle_path]
        ret = subprocess.run(command).returncode
        if ret == 0:
            return True
        return False
