...
class FfmpegHelper:
    ...
    def get_thumb(video_path: str, image_path: str, frames: str = None):
        ...
        if not video_path or not image_path:
            return False
        cmd = 'ffmpeg -hwaccel cuda -i "{video_path}" -ss {frames} -vframes 1 -f image2 "{image_path}"'.format(video_path=video_path,
                                                                                                              frames=frames,
    ...
    def extract_wav(video_path: str, audio_path: str, audio_index: str = None):
        ...
        # 提取指定音频流
        if audio_index:
            command = ['ffmpeg', '-hwaccel', 'cuda', "-hide_banner", "-loglevel", "warning", '-y', '-i', video_path,
    ...
    def extract_subtitle(video_path: str, subtitle_path: str, subtitle_index: str = None):
        ...
        if subtitle_index:
            ...
            command = ['ffmpeg', '-hwaccel', 'cuda', "-hide_banner", "-loglevel", "warning", '-y', '-i', video_path, subtitle_path]
        ret = subprocess.run(command).returncode
        if ret == 0:
            return True
        return False
